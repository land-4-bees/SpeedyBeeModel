...
#create raster that is all zero except for one pixel equal to 1
  ...
  landcover_table <- 'D:/Documents/StateIndicesAzavea/landmodel_inputs/Koh_LU_table_APIS.csv'
  if (forage_range == '5km') {
  ...
  #pesticide tables
  pesticide <- read.csv(paste0('D:/Documents/StateIndicesAzavea/landmodel_inputs/PestReclass/PA_CDL_PestReclass', insyear,'.csv'), stringsAsFactors = F)
  ...
  if (forage_range == '5km') {
    output_dir  <- paste0('./', state, "_5km_res", c.size, "_", year, '/')
  } else if (forage_range == '3km') {
    output_dir  <- paste0('./', state, "_3km_res", c.size, "_", year, '/')
  }
