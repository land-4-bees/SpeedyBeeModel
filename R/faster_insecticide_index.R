output_dir <- './sampledata/output'
landcover_path <- './sampledata/Cty18041.tif'
pesticide_table <- read.csv('./sampledata/Insecticide_CA_2012.csv')
forage_range <- NA
guild_table <- './sampledata/guild_table.csv'
ins_method <- 'mean'
lg_raster <- F
agg_factor=NA

normalize <- F; useW <- F; check_pesttable <- F
  

library(dplyr); library(profvis)
...
if (lg_raster == T & !is.na(agg_factor)) {
...
#if aggregation factor is supplied for large raster, reduce land use raster resolution
if (lg_raster == T & !is.na(agg_factor)) {
...
message("Finished intermediate raster calculations. Starting distance weighting (moving window) analysis.")

#####calculate distance weighted pesticide index

#window_sum <- raster::focal(x=mask_land, w=effdist.v, pad=T, padValue=0, na.rm=T) 
...
message("Finished insecticide raster moving window calculation.")
...
#write output raster
raster::writeRaster(simp.ins, paste0(output_dir, "/insecticideDW_FAST", land_name, ".tif"), overwrite=T)
